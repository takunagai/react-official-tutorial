...
4. 状態の管理：アプリが大きくなると、多くの状態がアプリの上部に近づく可能性がある。下の多くの離れたコンポーネントはそれを変更したいかもしれません。複雑な状態を管理し、それほど面倒なことなく離れたコンポーネントに渡すために、Context と一緒にレデューサーを使用するのが一般的

* コンテキストは静的な値に限定されない。次のレンダリングで別の値を渡すと、React は以下でそれを読み取るすべてのコンポーネントを更新する。これが、コンテキストが状態と組合せて使用されることが多い理由
* 一般に、ツリーのさまざまな部分にある離れたコンポーネントで情報が必要な場合は、コンテキストが役立つ
...
* レデューサーとコンテキストを組み合わせる手順
  ...
  3. ツリー内の任意の場所でコンテキストを使用
    ...
    - タスクリストを更新するために、任意のコンポーネントがコンテキストからディスパッチ関数を読み取り、それを呼び出すことができる
    - TaskApp コンポーネントはイベントハンドラーを渡さない。また、TaskList はイベントハンドラーをタスクコンポーネントに渡さない。各コンポーネントは、必要なコンテキストを読み取る
