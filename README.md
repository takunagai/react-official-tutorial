...
* イベントハンドラーを props として渡す
  - 多くの場合、親コンポーネントで子のイベントハンドラーを指定する必要がある。
  - 例えば、Button コンポーネントを使用している場所によっては、別の機能を実行したい場合があります。たとえば、ある機能で映画を再生し、別の機能で画像をアップロードします。これを行うには、次のように、コンポーネントが親から受け取る小道具をイベントハンドラーとして渡す。
